...
public class mediscreenPatientControllerTest {
	...
	public void testDeletePatient() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.delete("/patient/{id}", 2)).andExpect(status().isOk());
	}

	@Test
	public void testCreate() throws Exception {
		mockMvc.perform(post("/patient").content(asJsonString(new Patient())).contentType(MediaType.APPLICATION_JSON)
	...
	public void testUpdatePatient() throws Exception {
		...
		mockMvc.perform(put("/patient/{id}", 2).content(asJsonString(patientToSave))
				.contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
				.andExpect(jsonPath("$.lastName", is("Pennarun"))).andExpect(jsonPath("$.firstName", is("Adam")))
				.andExpect(jsonPath("$.dob", is("2014-12-24"))).andExpect(jsonPath("$.sex", is("M")))
				.andExpect(jsonPath("$.address", is("75020 Paris"))).andExpect(jsonPath("$.phone", is("888-88-88")));
	}
